...
void led_button_init(void) {
    ...
    PORTAbits.RA0 = 0;
    PORTAbits.RA1 = 0;
    xEventGroupClearBits((1 << 2));
}

void button1_handler(void) {
    PORTAbits.RA7 ^= 1;
    xEventGroupSetBits((1 << 2));
}
...
# 109 "main.c"
            ...
            if (endpoint == 0x03) {
                if (data[17] == 0) {
                    if (data[18] & (1 << 5)) PORTAbits.RA7 = ((data[18] & (1 << 0)) != 0);
                    if (data[18] & (1 << 6)) PORTAbits.RA0 = ((data[18] & (1 << 1)) != 0);
                    if (data[18] & (1 << 7)) PORTAbits.RA1 = ((data[18] & (1 << 2)) != 0);
                } else if (data[17] == 1) {
                    relay_node_addr = ((uint16_t)(((data[18]) & 0x00FF) + (((data[19]) & 0x00FF) << 8)));
                    xEventGroupSetBits((1 << 3));
                }
            } else if (endpoint == 0x02) {
                PORTAbits.RA7 = ((data[18] & (1 << 0)) != 0);
                PORTAbits.RA0 = ((data[18] & (1 << 1)) != 0);
                PORTAbits.RA1 = ((data[18] & (1 << 2)) != 0);
            }
        }
            break;
        case 0x45C0:
            if (data[0] == 0x07) xEventGroupSetBits((1 << 0));
            break;
        default:
            break;
    }
...
void znp_send_msg(uint16_t dst_addr, uint8_t endpoint, _Bool wait_for_rsps) {
    uint8_t* data = (0);
    uint8_t len = 0;
    if (endpoint == 0x02) {
        button_data.button1 = PORTAbits.RA7;
        button_data.button2 = PORTAbits.RA0;
        button_data.button3 = PORTAbits.RA1;
...
void main(void) {
    ...
    if (!PORTBbits.RB7) eeprom_write(0, 0x00);

    button_data.raw[1] |= 0xE0;

    uint8_t relay_node_lsb_addr
            = (uint8_t) eeprom_read(2);
    uint8_t relay_node_msb_addr
            = (uint8_t) eeprom_read(3);
    relay_node_addr
            = ((uint16_t)(((relay_node_lsb_addr) & 0x00FF) + (((relay_node_msb_addr) & 0x00FF) << 8)));
    ...
    while (1) {
        ...
        if (uxBits & (1 << 1)) {
            znp_send_msg(0x0000, 0x02, 1);
            xEventGroupClearBits((1 << 1));
        }

        if (uxBits & (1 << 2)) {
            if (relay_node_addr != 0)
                znp_send_msg(relay_node_addr, 0x02, 1);
            xEventGroupClearBits((1 << 2));
        }

        if (uxBits & (1 << 3)) {
            eeprom_write(2, ((relay_node_addr) & 0xFF));
            eeprom_write(3, (((relay_node_addr) >> 8) & 0xFF));
            xEventGroupClearBits((1 << 3));
        }

        if (sys_tick_get_tick() - keepalive_tick >= 20) {
            keepalive_tick = sys_tick_get_tick();
            znp_send_msg(0x0000, 0x04, 0);
            led_ind_on();

...
# 17 "./it_handle.h"
    ...
    typedef void (*ioc_rb1_isr_t)(void);
    typedef void (*ioc_rb2_isr_t)(void);
    typedef void (*ioc_rb3_isr_t)(void);
    typedef void (*ioc_rb4_isr_t)(void);
    typedef void (*ioc_rb5_isr_t)(void);
    typedef void (*ioc_rb6_isr_t)(void);
    typedef void (*ioc_rb7_isr_t)(void);
...
static ade_zx_cb_t _ade_zx_cb = (0);
static uint16_t _linecyc = 0;
static float _energy = 0;
static _Bool _is_reversed = 0;
...
void ade_config(ade_config_t* config) {
    ...
    if (_ade_zx_cb == (0) && config->zx_cb != (0)) {
    ...
    ade_write(0x0D, config->ch1os.reg_val, 1);
    ade_write(0x0E, config->ch2os.reg_val, 1);
    ade_write(0x18, config->irmsos, 2);
    ade_write(0x19, config->vrmsos, 2);
    ade_write(0x09, config->mode.reg_val, 2);
    ade_write(0x0A, config->irq_en.reg_val, 2);
    ade_read(0x0C, 2);
}
...
void ade_set_GAIN(ade_GAIN_reg_t gain_config) {
    ade_write(0x0F, gain_config.reg_val, 1);
}

void ade_set_LINECYC(uint16_t linecyc) {
    _linecyc = linecyc;
    ade_write(0x1C, linecyc, 2);
}

uint24_t ade_get_IRMS(void) {
    return (uint24_t) ade_read(0x16, 3);
}

uint24_t ade_get_VRMS(void) {
    return (uint24_t) ade_read(0x17, 3);
}

int ade_get_LAENERGY(void) {
    return ade_signed_value(ade_read(0x04, 3), 24);
}

uint16_t ade_get_PERIOD(void) {
    return (uint16_t) ade_read(0x27, 2);
}
...
void ade_get_power_meter_data(ade_power_meter_data_t* pData) {
    ...
    pData->irms =
            irms * (float) (1 / 52405.96386) + ((laenergy == 0) ? 0 : (float) (-0.006677591));

    if (vrms < 564590)
        pData->vrms = vrms * (float) (1 / 2911.207692) + (float) (-0.733192211);
    else
        pData->vrms = vrms * (float) (1 / 2645.024689) + (float) (-20.25007624);

    if (pData->irms < 0) pData->irms = 0;
    if (pData->vrms < 0) pData->vrms = 0;

    float line_periods = ((float) period) * 8 / 3579545;
    float accumulation_times = line_periods * _linecyc / 2;
    pData->active_power =
            ((_is_reversed ? -1 : 1) * laenergy / accumulation_times)
            * (float) (0.859830416) + ((laenergy == 0) ? 0 : (float) (0.257707374));
}

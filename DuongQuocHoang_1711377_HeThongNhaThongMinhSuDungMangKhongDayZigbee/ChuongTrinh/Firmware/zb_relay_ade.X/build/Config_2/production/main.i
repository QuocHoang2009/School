...
# 35 "main.c"
EventBits_t uxBits;
ade_config_t ade_cfg = { .gain.reg_val = 0x00, .mode.reg_val = 0x000C, .irq_en.reg_val = 0x0040, .linecyc = 150, .ch1os.reg_val = 0x00, .ch2os.reg_val = 0x00, .irmsos = 0x000, .vrmsos = 0x000, .is_reversed = 0, .zx_cb = (0), };
...
void znp_msg_handler(uint16_t cmd, uint8_t* data, uint8_t len) {
    switch (cmd) {
        case 0x4481:
            if ((data[6] == data[7]) && (data[6] == 0x03)) {
                if (data[17]) PORTDbits.RD2 = 0;
                else PORTDbits.RD2 = 1;
                xEventGroupSetBits((1 << 2));
            }
            break;
        case 0x45C0:
...
void znp_send_msg(uint8_t endpoint, _Bool wait_for_rsps) {
    uint8_t* data = (0);
    uint8_t len = 0;
    if (endpoint == 0x02) {
        dev_data.relay_state = !PORTDbits.RD2;
        data = dev_data.raw;
